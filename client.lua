...
Citizen.CreateThread(function()
	TriggerServerEvent('BadgerBankRobbery:CheckPerms')
end)

RegisterNetEvent('BadgerBankRobbery:bankblipalert')
AddEventHandler('BadgerBankRobbery:bankblipalert', function()
	...
	for _, ammunationcoords in pairs(config.ammunationcoords) do
		if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, ammunationcoords.x, ammunationcoords.y, ammunationcoords.z, true) < 5.0 then
	...
	for _, ltdcoords in pairs(config.ltdcoords) do
		if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, ltdcoords.x, ltdcoords.y, ltdcoords.z, true) < 5.0 then
	...
	for _, liquorcoords in pairs(config.liquorcoords) do
		if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, liquorcoords.x, liquorcoords.y, liquorcoords.z, true) < 5.0 then
			...
			if (config.enableBanks == true) then
				if GetDistanceBetweenCoords(coords.x, coords.y, coords.z, bankcoords.x, bankcoords.y, bankcoords.z, true) < 5.0 then
					...
					if IsControlJustReleased(0, 38) then -- E key
						...
						TriggerServerEvent('BadgerBankRobbery:bankrobberyalert')
						if (config.displayBlips == true) then
							bankcoords.blip = AddBlipForCoord(bankcoords.x, bankcoords.y, bankcoords.z)
							SetBlipSprite(bankcoords.blip, 353)
							SetBlipFlashTimer(bankcoords.blip, 1000 * config.timeToRob)
